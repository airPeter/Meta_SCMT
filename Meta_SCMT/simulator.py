    ...
    def __init__(self, **keyword_args) -> None:
        ...
        if not os.path.exists(gp_path):
            ...
            if not self.GP == saved_GP:
                warnings.warn(
                    'Your global parameters have changed. be careful loading any cached data, it may be in consist!')
        ...
        if self.GP.dim == 1:
            ...
            self.fitting_neffs = Fitting_neffs(
                self.GP.modes, self.gen_modes, self.GP.dh, self.GP.path)
            self.fftting_C = Fitting_C_matrix_1D(
                self.gen_modes, self.GP.modes, self.GP.res, self.GP.dh, self.GP.dx, self.GP.Knn, self.GP.path)
            self.fftting_E = Fitting_E_field_1D(
                self.gen_modes, self.GP.modes, self.GP.res, self.GP.dh, self.GP.dx, self.GP.Knn, self.GP.path)
        ...
        if self.GP.dim == 2:
            if self.GP.modes > 1:
                raise Exception(
                    "Currently, only support modes = 1 for 2 dimensional waveguide.")
            self.gen_modes = Gen_modes2D(self.GP)
            self.fitting_neffs = Fitting_neffs(
                self.GP.modes, self.gen_modes, self.GP.dh, self.GP.path)
            self.fftting_C = Fitting_C_matrix_2D(
                self.gen_modes, self.GP.modes, self.GP.res, self.GP.dh, self.GP.dx, self.GP.Knn, self.GP.path)
            self.fftting_E = Fitting_E_field_2D(
                self.gen_modes, self.GP.modes, self.GP.out_res, self.GP.dh, self.GP.dx, self.GP.Knn, self.GP.path)
