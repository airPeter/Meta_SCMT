...
class Fullwave_1D_2layer():
    ...
    def tidy3d_init_sim(self, prop_dis: Union[float, List[float]], N: int, hs: Union[np.ndarray, List[np.ndarray]], res: Optional[int] = None, theta: float = 0, empty: bool = False) -> None:
        ...
        self.N = N
        self.prop_dis = prop_dis
        self.focal_length = prop_dis[-1]
        xh = N * self.GP.period
        NA = np.sin(xh / np.sqrt(xh**2 + self.focal_length**2))
        print(f"numerical aperture: {NA:.2f}")
        self.min_focal_spot = self.GP.lam / 2 / NA
        self.efficiency_length = 6 * self.min_focal_spot
        # Simulation domain size (in micron)
        spacing = 2 * self.GP.lam
        z_size = self.GP.wh + 2 * spacing + prop_dis[0] + prop_dis[1]
        ...
        if self.GP.n_sub != 1:
            ...
            sub = td.Structure(
                geometry = td.Box(center=[0, 0, z_plane + self.GP.wh + prop_dis[0]/2],
                         size=[td.inf, td.inf, prop_dis[0]]),
                medium = material2,
                name = 'substrate')
        ...
        if not empty:
            for i in range(N):
                ...
                width = hs[1][i]
                temp_wg = td.Structure(
                    geometry = td.Box(center=[x, 0, z_plane + 3 * self.GP.wh/2 + self.prop_dis[0]],
                                 size=[width, td.inf, self.GP.wh]),
                    medium = material1,
                    name = f'wg1_: {i}')
                waveguides.append(temp_wg)
        ...
        monitor_far = td.FieldMonitor(
            center=[0, 0, z_plane + 2 * self.GP.wh + prop_dis[0] + prop_dis[1]], size=[x_wgs, 0, 0], freqs=[fcen], name = 'far')
        monitor_near = td.FieldMonitor(center=[
                                      0, 0, z_plane +  2 * self.GP.wh + prop_dis[0] + self.GP.lam/2], size=[x_wgs, td.inf, 0], freqs=[fcen], name = 'near')
        monitor_in = td.FieldMonitor(
            center=[0, 0, -z_size/2 + 0.7 * spacing], size=[x_wgs, td.inf, 0], freqs=[fcen], name = 'in')
        
        monitor_far_flux = td.FluxMonitor(
            center=[0, 0, z_plane + 2 * self.GP.wh + prop_dis[0] + prop_dis[1]], size=[x_wgs, td.inf, 0], freqs=[fcen], name = 'far_flux')

        monitor_near_flux = td.FluxMonitor(center=[
                                      0, 0, z_plane +  2 * self.GP.wh + prop_dis[0] + self.GP.lam/2], size=[x_wgs, td.inf, 0], freqs=[fcen], name = 'near_flux')
        monitor_in_flux = td.FluxMonitor(
            center=[0, 0, -z_size/2 + 0.7 * spacing], size=[x_wgs, td.inf, 0], freqs=[fcen], name = 'in_flux')
        monitor_focus_flux = td.FluxMonitor(center=[0, 0, z_plane + 2 * self.GP.wh + prop_dis[0] + prop_dis[1]], size=[
                                       self.efficiency_length, td.inf, 0], freqs=[fcen], name = 'focus_flux')
        monitor_back_flux = td.FluxMonitor(
            center=[0, 0, -z_size/2 + 0.2 * spacing], size=[x_wgs, td.inf, 0], freqs=[fcen], name = 'back_flux')
