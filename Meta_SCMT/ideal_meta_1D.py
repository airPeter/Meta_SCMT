...
from .SCMT_utils.SCMT_model_1D import Ideal_model
import torch
...
class Ideal_meta_1D():
    ...
    def init_sim(self, prop_dis: float, N: int, ideal_lens: bool = True, res: Optional[int] = None, theta: float = 0, vis_path: Optional[str] = None) -> None:
        '''
        input:
            theta: [rad]
        '''
        ...
        x_lens, phase_lens = lens_1D(
            int(round(x_size * self.res)), 1/self.res, self.prop_dis, 2 * np.pi / self.GP.lam)
        ...
        self.stop_condition_func = mp.stop_when_fields_decayed(
            dt=prop_dis, c=mp.Ez, pt=mp.Vector3(0, y_plane + prop_dis), decay_by=1e-5)
...
class Ideal_meta():
    ...
    def model_init(self, N: int, prop_dis: float, init_phase: Optional[np.ndarray] = None, lens: bool = False) -> None:
        self.total_size = (N) * self.GP.res
        if init_phase is None and lens == True:
            _, init_phase = lens_1D(
                self.total_size, self.GP.dx, prop_dis, self.GP.k)
    ...
    def forward(self, E0: Optional[np.ndarray] = None, theta: float = 0, vis: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        ...
        If = If.cpu().numpy()
        I_out = If.sum()
        phy_x = (np.arange(self.total_size) -
                 (self.total_size - 1)/2) * self.GP.dx
        print(
            f"I_in: {I_in:3f}, I_out: {I_out:3f}, I_out/I_in: {I_out/I_in:3f}.")
        if vis:
            plt.figure()
            plt.plot(phy_x, If)
            plt.show()
