...
class Fitting_C_matrix_2D():
    ...
    def fit(self, layers = 4, nodes = 128, steps = 1000, lr = 0.001, vis = True, load = True, save_fig = False):
        '''
            we use fully connected network (FCN) to fit a function that given the distance and the widths of two pillars i and j, output Cij\
            ...
            Input:
                ...
                load: if true, read the C_map.npy and C_input.npy from caches, which are training dataset. else: the dataset will be generated from skratch.
