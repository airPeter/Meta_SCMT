...
class PBA():
    ...
    def design_lens(self, N, focal_length, vis=True, quarter=False):
        ...
        for idx, lam in enumerate(self.GP.lams):
            ...
            if self.dim == 1:
                x_lens, lens = lens_1D(
                    N, self.GP.period, focal_length, 2 * np.pi / lam)
            elif self.dim == 2:
                if quarter:
                    N2 = 2 * N
                    x_lens, lens = lens_2D(
                        N2, self.GP.period, focal_length, 2 * np.pi / lam)
                    x_lens = x_lens[N:]
                    lens = lens[N:, N:]
                else:
                    x_lens, lens = lens_2D(
...
def gen_width_from_phase(width_phase_maps: List, target_phases: List):
    ...
    assert (len(phases) == len(target_phases)
            ), "set of phases in width phase map should be equal to set of target phases"
