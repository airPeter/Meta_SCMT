...
from .PBA_model_1D_lam import PBA_model, PBA_model_2_layer
from ..SCMT_utils.SCMT_1D import plot_hs, plot_FFT_I, plot_If, gen_x_angle, loss_max_center, loss_max_range
import torch
...
class PBA_1D():
    ...
    def optimize(self, notes, steps, lr=0.01, minmax=True, loss_weights: List = None, target='lens', **kargs):
        ...
        if deflecting_angle is not None:
            ...
            if type(self.prop_dis) == list:
                print("using the last layer prop dis to calcuate NA.")
                NA = radius / np.sqrt(radius**2 + self.prop_dis[-1]**2)
            else:
